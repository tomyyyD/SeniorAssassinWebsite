var players = {    
    "player":[
        /*
        Sample Player Entry
        {
            "name":"Victor Momney",
            "isAlive":false,
            "diedToday":false,
        }
        */
        {
           "name":"Miranda Acosta",
           "isAlive":false,
           "diedToday":false,
           "isSafe":false,
        },
        {
            "name":"James Armstrong",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Brooke Aijala",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Bradleigh Barcus",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Bobby Bell",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Lilli Bergman",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Victoria Blancarte",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Caleigh Bold",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Carlee Bonanno",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Preston Brindle",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Tim Brooks",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Natalie Brown",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Kate Callahan",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Bre Callahan",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Charlotte Camp",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Sydnee Campbell",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Will Cano",
            "isAlive":true,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Will Cody",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Jason Commentucci",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"August Crane",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Thomas Damiani",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Quinn Defibaugh",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Josh Devine",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Finlay Drucquer",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Mary-Kate Duffy",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Kyra Emery",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        { 
            "name":"Gryphen Ferguson",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Sam Ferreira",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Morgan Flaim",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Danny Flogaus",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Julia Forrest",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Taylor Freeze",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Sarah Gautreau",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Dylan Gazda",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Courtney Glass",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Alyssa Greiner",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Avery Grimm",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Emma Gubanich",
            "isAlive":true,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Caroline Halsey",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Liam Harrington",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Jon Havrilak",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Brandon Hays",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Lillian Heckler",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Connor Heft",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Cali Hensley",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Sebastian Herrada",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Evan Hild",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Megan Himmelreich",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Scott Hoffman",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Colin Horger",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Kyle Isleib",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Will Jaeger",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Carson Johnson",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Nathaniel Jones",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"James Joyce",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Ellie Kalbach",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Ellie Koitzsch",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Rylee Laloup",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Bri Landwersiek",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Izzy LeBlanc",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Tommy Lewmen",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Zachary Lineman",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Alex Lobello",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Taylor Marburger",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Chelsea McNeff",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Rachel Metz",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Ethan Miller",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Dean Mihos",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Victor Mominey",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Grace Moser",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Katie Murray",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        { 
            "name":"Lauren Neuhoff",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Abby Norton",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Aidan O'Neil",
            "isAlive":true,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Alina Orzechowski",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Anant Pothakamury",
            "isAlive":true,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Lincoln Rajsky",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Laura Rivkin",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Jenny Ruotolo",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Ethan Sabatino",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },       
        {
            "name":"Katelyn Scheeler",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Aston Shrum",
            "isAlive":true,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Gracyn Smith",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Sydney Smith",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Grace Storti",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Bennet Stutzmen",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Anna Subick",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Cole Swierczek",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Sydney Trapp",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Faith Turnbull",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Ella Vandruff",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Parker Waldman",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Blaine Waltimeyer",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        }, 
        {
            "name":"Morgan Weaver",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Avery White",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Tara Wittlinger",
            "isAlive":true,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Kaily Zakarevicz",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
        {
            "name":"Lauren Zelinske",
            "isAlive":false,
            "diedToday":false,
            "isSafe":false,
        },
    ]
};

window.onload = e => {
    let playerCounter = 0;
    for (let i = 0; i < players.player.length; i++){
        let newPlayer = document.createElement("li");
        let divider = document.createElement('hr');

        divider.style.width = "85%";
        newPlayer.style.textAlign = "center";

        let playerLiText = document.createTextNode(players.player[i].name)
        
        newPlayer.appendChild(playerLiText);

        if (players.player[i].isSafe){
            const safeList = document.getElementById("safeList");

            let safePlayer = document.createElement("li");
            safePlayer.style.textAlign = "center";

            let safePlayerText = document.createTextNode(players.player[i].name);
            safePlayer.appendChild(safePlayerText);

            safeList.appendChild(safePlayer);
        }

        if (players.player[i].isAlive){    
            const aliveList = document.getElementById("alivePlayerList");

            aliveList.appendChild(newPlayer);
            aliveList.append(divider);
            
            playerCounter++;
        }else{
            const deadList = document.getElementById("deadPlayerList");
            if (players.player[i].diedToday){
                let newPlayer2 = document.createElement("li");
                newPlayer2.style.textAlign = "center";

                let playerLiText2 = document.createTextNode(players.player[i].name)
                newPlayer2.appendChild(playerLiText2);
        
                const todayList = document.getElementById("todayDeathList");

                deadList.appendChild(newPlayer);
                deadList.append(divider);
                todayList.appendChild(newPlayer2);
            }else{
                deadList.appendChild(newPlayer);
                deadList.append(divider);
            }
        }
    }
    document.getElementById("playerCounter").innerHTML = playerCounter;
};
